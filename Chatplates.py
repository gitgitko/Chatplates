"""
Chatplates - WebSocket Server v5 (with Guest Lounge)
Run: uvicorn Chatplates:app --host 0.0.0.0 --port 8000 --reload
"""
...
async def register_user(ws, name: str, is_owner: bool, room_id: str):
    ...
    await ws.send_text(json.dumps({"type": "auth_ok", "name": name, "is_owner": is_owner}))
    ...
    try:
        while True:
            ...
            if t == "message":
                ...
                if not invited_name:
                    await ws.send_text(json.dumps({"type": "system", "text": "Name required.", "time": now()}))
                    continue
                code = gen_invite()
...
async def guest_endpoint(ws: WebSocket):
    ...
    await ws.send_text(json.dumps({"type": "system", "text": f"Welcome, {name}! You joined the Guest Lounge.", "time": now()}))
    await broadcast(room_id, {"type": "system", "text": f"{name} joined.", "time": now()}, exclude=name)
    await broadcast_user_list(room_id)
    await send_online_list()

    # Message loop for guests
    try:
        while True:
            raw = await ws.receive_text()
            data = json.loads(raw)
